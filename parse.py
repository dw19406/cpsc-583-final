...
def parse_method(args, dataset, n, c, d, device):
    if args.method == 'gcn':
        model = GCN(in_channels=d,hidden_channels=args.hidden_channels,out_channels=c,num_layers=args.num_layers,use_bn=args.use_bn).to(device)
    elif args.method == 'gat':
        model = GAT(d, args.hidden_channels, c, num_layers=args.num_layers, use_bn=args.use_bn, heads=args.gat_heads, out_heads=args.out_heads).to(device)
    elif args.method=='gat2':
        model = GAT2(d, args.hidden_channels, c, num_layers=args.num_layers, use_bn=args.use_bn, heads=args.gat_heads, out_heads=args.out_heads).to(device)
